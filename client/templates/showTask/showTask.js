...
Template.showTask.helpers({
  taskCompletion(){
    if (Tasks.findOne(FlowRouter.getParam('_id')).opened === false)
      return "warning";
    let result = Attempts.find({task: FlowRouter.getParam('_id'), userId: Meteor.userId()}).count();
    if (result === 0)
      return "default";
  ...
  winners(){
    let winners = Attempts.find({task: FlowRouter.getParam('_id'), success: true}).map(attempt => attempt.user).join(", ");
